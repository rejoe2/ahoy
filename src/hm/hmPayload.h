        ...
        void process(bool retransmit) {
            for (uint8_t id = 0; id < mSys->getNumInverters(); id++) {
                ...
                if (!mPayload[iv->id].complete) {
                    ...
                    if ( (retransmit) && (mPayload[iv->id].retransmits < mMaxRetrans) ) { //mPayload[iv->id].requested) && (!mPayload[iv->id].rxTmo) ) {// (!mPayload[iv->id].rxTmo) && (!pyldComplete) ) { //(mPayload[iv->id].requested) &&
