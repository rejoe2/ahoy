            ...
            get([this](bool valid, const queue_s *q) {
                ...
                switch(mState) {
                    case States::RESET:
                        ...
                        // add function to get expected frame count.
                        mFramesExpected = getFramesExpected(q);
                        //mTimeout     = (q->iv->ivGen == IV_MI) ? MI_TIMEOUT : (((q->iv->mGotFragment && q->iv->mGotLastMsg) || mIsRetransmit) ? SINGLEFR_TIMEOUT : ((q->cmd != AlarmData) && (q->cmd != GridOnProFilePara) ? DEFAULT_TIMEOUT : (1.5 * DEFAULT_TIMEOUT)));
                    ...
                    case States::START:
                        ...
                        //mWaitTime.startTimeMonitor(mTimeout); //review
